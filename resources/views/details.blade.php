        ...
        <div class="col-lg-5">
          ...
          <div class="product-single__addtolinks">
            @if(Cart::instance("wishlist")->content()->Where('id',$product->id)->count()>0)
             <form id="frm-remove-item" method="POST" action="{{route('wishlist.item.remove', ['rowId'=>Cart::instance("wishlist")->content()->Where('id',$product->id)->first()->rowId])}}">
              @csrf
              @method('DELETE')
                ...
                <div class="product-card__price d-flex">
                    <span class="money price">
                        @if($rproduct->sale_price)
                            <s>S/{{ $rproduct->regular_price }}</s> S/{{ $rproduct->sale_price }}
                        @else
                            S/{{ $rproduct->regular_price }}
                        @endif
                    </span>
                </div>
